...
metadata_cl = metadata[metadata$SampleType == "CellLine",]
counts_full_ensembl_cl = counts[,colnames(counts) %in% metadata_cl$miRNA_ENCODE_Data_ID]
metadata_ti_sorted=metadata_cl[order(match(metadata_cl$miRNA_ENCODE_Data_ID,colnames(counts_full_ensembl_cl))),]
table(metadata_cl$miRNA_ENCODE_Data_ID == colnames(counts_full_ensembl_cl))
save(counts_full_ensembl_cl, metadata_cl, file="rdata/counts_full_ensemblIDs_cellLines.rda")

metadata_dcl = metadata[metadata$SampleType == "DifferentiatedCells",]
counts_full_ensembl_dcl = counts[,colnames(counts) %in% metadata_dcl$miRNA_ENCODE_Data_ID]
metadata_ti_sorted=metadata_dcl[order(match(metadata_dcl$miRNA_ENCODE_Data_ID,colnames(counts_full_ensembl_dcl))),]
table(metadata_dcl$miRNA_ENCODE_Data_ID == colnames(counts_full_ensembl_dcl))
save(counts_full_ensembl_dcl, metadata_dcl, file="rdata/counts_full_ensemblIDs_differentiatedCLs.rda")
...
counts_full_geneid_cl = counts_geneid[,colnames(counts_geneid) %in% metadata_cl$miRNA_ENCODE_Data_ID]
counts_full_geneid_dcl = counts_geneid[,colnames(counts_geneid) %in% metadata_dcl$miRNA_ENCODE_Data_ID]
...
save(counts_full_geneid_cl, metadata_cl, file="rdata/counts_full_geneids_cellLines.rda")
save(counts_full_geneid_dcl, metadata_dcl, file="rdata/counts_full_geneids_differentiatedCLs.rda")
...
counts_0_geneid_cl = counts_greaterthan0_func(counts_full_geneid_cl)
counts_0_geneid_dcl = counts_greaterthan0_func(counts_full_geneid_dcl)
...
save(counts_0_geneid_cl, metadata_cl, file="rdata/counts_0_geneids_cellLines.rda")
save(counts_0_geneid_dcl, metadata_dcl, file="rdata/counts_0_geneids_differentiatedCLs.rda")
