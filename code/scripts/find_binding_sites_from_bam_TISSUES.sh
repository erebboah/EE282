...
cd "$path"/bam/tissues
for i in `ls -1v *.bam`
	do bedtools intersect -bed -abam "$path"/bam/tissues/${i%} \
	-b "$path"/ref/Targets_miR-"$mirna_number".bed | awk -F "\t" '{print $1 "\t" $7 "\t" $8}' > "$path"/output/tissues/miR-"$mirna_number"_intersect_${i%}.bed
done
...
num_reads_total=$(for i in `ls -1v *.bam`; do
          samtools view -c "$path"/bam/tissues/${i%}
      done)

cd "$path"/bam/tissues
num_reads_intersect=$(for i in `ls -1v *.bam`; do
          wc -l "$path"/output/tissues/miR-"$mirna_number"_intersect_${i%}.bed
      done)

paste <(printf %s "$num_reads_total") <(printf %s "$num_reads_intersect") > "$path"/output/tissues/miR-"$mirna_number"_NumReadOverlaps.txt

Rscript "$path"/scripts/percent_bindingSite_barplot_TISSUES.R -d "$path"/output/tissues/miR-"$mirna_number"_NumReadOverlaps.txt -o "$path"/output/tissues -n "$mirna_number"
